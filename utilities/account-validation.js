...
validate.registationAddNewVehicle = () => {
  return [
    // maket is required and must be string
    body("inv_make")
      .trim()
...
validate.checkAddNewVehicle = async (req, res, next) => {
  const { inv_make, inv_model, inv_year, inv_image, inv_thumbnail, inv_price, inv_miles, inv_color} = req.body
  let errors = []
  errors = validationResult(req)
  if (!errors.isEmpty()) {
    let nav = await utilities.getNav()
...
validate.registationRequestQuote = () => {
  return [
    ...
    body("quote_email")
      ...
      const emailExists = await accountModel.checkExistingEmailQuote(quote_email)
      if (emailExists){
        throw new Error("Email exists. Please log in or use different email")
      }
    }),
